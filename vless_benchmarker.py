...
async def tcp_ping(server: VLESSServer, timeout: int) -> bool:
    try:
...
async def pre_filter_servers(servers: List[VLESSServer], timeout: int, workers: int, progress: Progress) -> List[VLESSServer]:
    ...
    async def check(s):
        async with semaphore:
            is_alive = await tcp_ping(s, timeout)
            progress.update(task_id, advance=1)
            return s if is_alive else None
...
async def main():
    ...
    if not os.path.exists(args.singbox_path):
